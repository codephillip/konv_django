...
shell:
	@echo 'To activate the venv use source ~/.venv/app/bin/activate . Use deactivate to exit'

lint:
	# starts a pipenv shell, shows autopep8 diff and then fixes the files
	# does the same for isort
	@echo '---Running autopep8---'
	${bin_path}/autopep8 app -r -d
	${bin_path}/autopep8 app -r -i
	@echo '---Running isort---'
	${bin_path}/isort app --diff
	${bin_path}/isort app --atomic
